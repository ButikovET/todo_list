  ...
  return (
    <div className="d-flex justify-content-between px-3 border_footer">
      <div className="align-self-center">
        {!filter
        ?`${itemsLength} ${filter} item${itemsLength === 1 ? '': 's'}${!itemsLength ? '': ' left'}`
        :filter=='active'
            ?`${activeLength} ${filter} item${activeLength === 1 ? '': 's'}${!activeLength ? '': ' left'}`
            :`${completedLength} ${filter} item${completedLength === 1 ? '': 's'}${!completedLength ? '': ' left'}`}
      </div>
