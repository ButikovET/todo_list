...
const TodoInput = ({addTask, onTextChange, taskText, selectAllTasks, isAllDone, itemsLength}) => {
    return (
        <form className="input-group shadow-sm bg-body rounded-0 h-100" onSubmit={(e)=>{e.preventDefault(); addTask()}}>
            {itemsLength?<span className={isAllDone==0?classes.down_arrow+" "+classes.color_gray:classes.down_arrow} id="basic-addon1" onClick={selectAllTasks}>&#8964;</span>:<></>}
